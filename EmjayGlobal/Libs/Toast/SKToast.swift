...
//            let blurEffect = UIBlurEffect(style: toastViewBackgroundStyle)
        ...
        toastView?.backgroundColor = UIColor(hexString: "#248C1B")
        /// Setup Message Label
        if statusLabel == nil {
    ...
    // MARK: - Hide
    public static func hideToast(afterDuration: DispatchTime){
        DispatchQueue.main.asyncAfter(deadline: afterDuration) {
            SKToast.shared.hideToastView()
        }
    }
    public static func hideTosst(){
       
        SKToast.shared.hideToastView()
        
    }
    fileprivate func hideToastView() {
        if self.toastView != nil && self.toastView?.alpha == 1 {
